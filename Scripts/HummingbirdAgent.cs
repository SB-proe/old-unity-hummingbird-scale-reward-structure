...
/// <summary>
/// A hummingbird MLA (Machine Learning Agent)
/// </summary>
public class HummingbirdAgent : Agent
{
    ...
    // Maximun angle the bird can pitch up or down
    ...
    // If distance bonus to nearest flower has been applied
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // Defaults to spawn in front of a flower
    ...
    /// <summary>
    /// Called when and action is received from either the player input or the nural network
    /// 
    ...
    /// Index 4: yaw angel (+1 = turn right, -1 = turn left)
    /// </summary>
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Observve the agent's local rotation (4 observations)
        sensor.AddObservation(transform.localRotation.normalized);

        // Get a vector from the beak tip to the nearest flower
        Vector3 toFlower = nearestFlower.FlowerCenterPosition - beakTip.position;

        // Observe a normalized bector pointing to the nearest flower (3 observations)
        sensor.AddObservation(toFlower.normalized);

        // Observe a dot product that indicates whether the beak tip is infront of the flower
        // (+1 menas that the beak tip is directly infront of the flower, -1 means directly behind)
    ...
    /// </summary>
    /// <param name="actionsOut">And ouput action array</param>
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        var continuousActionsOut = actionsOut.ContinuousActions;
        // Create placeholds for all movement/turning
        ...
        // All vlaues should be between -1 and +1
        ...
        // Combind the movement vectors and normalize
        Vector3 combind = (forward + left + up).normalized;

        // Add the 3 movement values, pitch, and yaw to the actionsOut array
        continuousActionsOut[0] = combind.x;
        continuousActionsOut[1] = combind.y;
        continuousActionsOut[2] = combind.z;
    ...
    /// <summary>
    /// Move the agent to a safe random position (ie does not collide with anything)
    ...
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        ...
        // Loop until a safe position is fount or we run out of attempts
        while(!safePositionFound && attemptsRemaining > 0)
        {
            ...
            if (inFrontOfFlower && flowerArea.Flowers.Count > 0)
            {
                ...
                // Position 10-20 cm infront of the flower
            ...
            else
            {
                ...
                // Combind height, radius, and direction to pick a potential position
    ...
    private void TriggerEnterOrStay(Collider collider)
    {
        ...
        if (collider.CompareTag("nectar"))
        {
            ...
            {
                // Look up thye flower for this nectar collider
                ...
                // Keep trak of nectar obtained
                NectarObtained += nectarReceived;

                if (trainingMode)
                {
                    // Calculate reward for getting nectar
                    float bonus = 0.04f * Mathf.Clamp01(Vector3.Dot(transform.forward.normalized, -nearestFlower.FlowerUpVector.normalized));
    ...
    {
        // Avoids scenario where the nearest flower nectar is stolen by opponent and not updated
        ...
        if (nearestFlower != null)
        {
            float distanceToFlower = Vector3.Distance(nearestFlower.transform.position, beakTip.position);
            if (distanceToFlower <= 0.75 && bonus == false)
            {
                bonus = true;
                AddReward(0.05f);
            } 
            else if (distanceToFlower > 0.75 && bonus == true)
            {
                bonus = false;
                AddReward(-0.05f);
            } 
